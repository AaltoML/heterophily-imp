...
# from ....hetGNN.utils import calculate_homophily
...
class GATConv(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        # scaling = calculate_homophily(x1, x2, het_mode = het_mode)
        scaling = calculate_homophily(x, edge_index, het_mode = het_mode)
            
        x = (x_src, x_dst)
        # propagate_type: (x: OptPairTensor, alpha: Tensor, scaling: Tensor)
        out = self.propagate(edge_index, x=x, alpha=alpha, size=size, scaling = scaling)
        if self.concat:
    ...
    def message(self, x_j: Tensor, alpha: Tensor, scaling: Tensor) -> Tensor:
        return alpha[:,:,None] * scaling[:,None,None] * x_j
