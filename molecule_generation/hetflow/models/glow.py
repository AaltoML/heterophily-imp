...
from hetflow.models.basic import ZeroConv2d, ActNorm, InvConv2dLU, InvConv2d, InvRotationLU, InvRotation, ActNorm2D
from hetflow.models.coupling import AffineCoupling, GraphAffineCoupling
...
class FlowOnGraph(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, masked_row, affine=True, graph_conv_option = None):
        ...
        self.coupling = GraphAffineCoupling(n_node, in_dim, hidden_dim_dict, masked_row, affine=affine, graph_conv_option= graph_conv_option)
...
class BlockOnGraph(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, n_flow, mask_row_size=1, mask_row_stride=1, affine=True, graph_conv_option = None):  #, conv_lu=True):
        """
        ...
        for i in range(n_flow):
            ...
            self.flows.append(FlowOnGraph(n_node, in_dim, hidden_dim_dict, masked_row=masked_row, affine=affine, graph_conv_option=graph_conv_option))
    ...
    def reverse(self, adj, output, layer_wise = False):
        input = output
        
        if layer_wise:
            input_list = [input]
            for flow in self.flows[::-1]:
                input_list.append(flow.reverse(adj, input))
            return input_list
        else:
            for flow in self.flows[::-1]:
...
class GlowOnGraph(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, n_flow, n_block,
                 mask_row_size_list=[2], mask_row_stride_list=[1], affine=True, graph_conv_option=None):  # , conv_lu=True): # in_channel: 2 default
        ...
        for i in range(n_block):
            ...
            self.blocks.append(BlockOnGraph(n_node, in_dim, hidden_dim_dict, n_flow, mask_row_size, mask_row_stride, affine=affine, graph_conv_option=graph_conv_option))
    ...
    def reverse(self, adj, z, layer_wise = False):
        # (bs, 4,9,9), zz: (bs, 9, 5)
        input = z
        for i, block in enumerate(self.blocks[::-1]):
            input = block.reverse(adj, input, layer_wise = layer_wise)
        return input
