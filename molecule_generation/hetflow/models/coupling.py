...
from hetflow.models.basic import GraphLinear, GraphConv, GraphConvHetero, ActNorm, ActNorm2D
...
class GraphAffineCoupling(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, masked_row, affine=True, graph_conv_option = None):
        ...
        for out_dim in self.hidden_dim_gnn:  # What if use only one gnn???
            if graph_conv_option is None:
                self.net.append(GraphConv(last_dim, out_dim))
            else:
                option = graph_conv_option[:-2]
                mode = graph_conv_option[-2:]
                if option == 'HetFlow':
                    self.net.append(GraphConvHetero(last_dim, out_dim, mode = mode))
                elif option == 'MoFlow':
                    self.net.append(GraphConv(last_dim, out_dim, mode = mode))
                else:
                    raise ValueError('graph_conv_option should be `MoFlow` or `HetFlow` + two binary digits!')
...
def test_AffineCoupling():
    from hetflow.models.model import rescale_adj
    torch.manual_seed(0)
    bs = 2
    nodes = 9
    ch = 5
