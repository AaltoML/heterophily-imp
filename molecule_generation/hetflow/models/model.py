...
from hetflow.models.glow import Glow, GlowOnGraph
...
class HetFlow(nn.Module):
    def __init__(self, hyper_params: Hyperparameters):
        ...
        if hyper_params.learn_dist == 0:
            self.register_buffer('ln_var', torch.zeros(1))
        elif hyper_params.learn_dist == 1:
            self.ln_var = nn.Parameter(torch.zeros(1))  # (torch.zeros(2))  2 is worse than 1
        elif hyper_params.learn_dist == 2:
            self.ln_var = nn.Parameter(torch.zeros(2))
        ...
        self.atom_model = GlowOnGraph(
            ...
            graph_conv_option = hyper_params.graph_conv_option
        )
    ...
    def reverse(self, z, true_adj=None, layer_wise = False):  # change!!! z[0] --> for z_x, z[1] for z_adj, a list!!!
        """
        ...
        with torch.no_grad():
            ...
            h_x = self.atom_model.reverse(adj_normalized, h_x, layer_wise=layer_wise)
            
            if self.noise_scale == 0:
                if layer_wise:
                    h_x = [ (h+0.5)*2 for h in h_x]
                else:
                    h_x = (h_x + 0.5) * 2
